...
public class BeaconMonitorManager extends ReactContextBaseJavaModule implements BeaconConsumer {
  ...
  private void addRangeNotifier() {
    beaconManager.addRangeNotifier(new RangeNotifier() {
      ...
      public void didRangeBeaconsInRegion(final Collection<Beacon> beacons,
                                          final Region region) {
        //TODO tratar retries por separado del tope de medidas.
        totalMeasures++;
        if (beacons != null && !beacons.isEmpty()) {
          Log.d(TAG, "Beacons found: " + beacons.size());
          beaconsRanged.addAll(beacons);
          if (totalMeasures == 10L) {
            totalMeasures = 0L;
  ...
  private WritableMap createResponse(@NonNull final Collection<Beacon> beacons,
    ...
    map.putArray("beacons", createWritableArray(beacons));

    return map;
  }

  private WritableArray createWritableArray(Collection<Beacon> beacons) {

    Map<String, List<Beacon>> map =
        beacons.stream().collect(groupingBy(Beacon::getBluetoothAddress));

    WritableArray a = new WritableNativeArray();

    for (String macAddress : map.keySet()) {

      List<Beacon> measuresByMacAddress = map.get(macAddress);

      Beacon beacon = measuresByMacAddress.get(0);

      Double avgDistance = getAverageDistance(measuresByMacAddress);

      Log.d(TAG, "Mac addr: " + beacon.getBluetoothAddress() + " - Distance: " + avgDistance);
      ...
      if (avgDistance == Double.POSITIVE_INFINITY
          || Double.isNaN(avgDistance)
          || avgDistance == Double.NEGATIVE_INFINITY) {
        b.putDouble(BEACON_DISTANCE, 999.0);
        b.putString(BEACON_PROXIMITY, "far");
      } else {
        b.putDouble(BEACON_DISTANCE, avgDistance);
        b.putString(BEACON_PROXIMITY, getProximity(avgDistance));
      }
      a.pushMap(b);
    }
  ...
  private Double getAverageDistance(final List<Beacon> measuresByMacAddress) {
    return measuresByMacAddress.stream()
            .mapToDouble(Beacon::getDistance).average().getAsDouble();
  }
